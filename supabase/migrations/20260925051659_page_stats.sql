-- page_stats: daily views of product pages, collections and on-site searches, for the Trend radar.
-- Aggregated in the database so the page never loads raw events. Product keys are URL handles
-- (locale prefixes and /collections/x/products/y are folded in). Search terms are returned still
-- URL-encoded; terms that look like an email or phone number are dropped here, and again in the app.

create function public.page_stats(p_from timestamptz, p_to timestamptz)
...
as $$
  with e as (
    select (e.occurred_at at time zone 'UTC')::date as day, e.session_id, e.title,
...
revoke execute on function public.page_stats(timestamptz, timestamptz) from public, anon, authenticated;
